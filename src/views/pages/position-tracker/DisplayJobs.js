...
const normalizeJob = (j) => ({
  ...
  title: j.title,
  company: j.company,
  // 1. Get the name from the nested User object
  client_name: j.Client?.user?.full_name || null, 
  // 2. THIS IS THE KEY: Match the user_id so the dropdown highlights it
...
const fetchJobs = async () => {
  try {
    const response = await getAllJobs();
    setJobs(response.map(normalizeJob));
  } catch (err) {
    console.error('Failed to fetch jobs:', err);
    setJobs([]);
  }
 ...
 const handleAssignClient = async (jobId, clientId) => {
  ...
  try {
    // Call API and get the updated job
    const res = await assignClientToJob({ jobId, clientId });

    // Get the assigned client name and job title from API response
    const assignedClientName = res.data.job.Client?.user?.full_name || '—';
    const jobTitle = res.data.job.title || 'this job';

    // Refresh jobs from DB
    const refreshed = await getAllJobs();
    setJobs(refreshed.map(normalizeJob));

    // Show alert with client name and job name
    setAlertMessage(`Client "${assignedClientName}" assigned to "${jobTitle}" successfully!`);
    setAlertColor('success');
    setShowAlert(true);
  } catch (err) {
    console.error('Assignment failed:', err);
   ...
   <CTableDataCell>
  <select
    value={j.assigned_client_id ?? ''} // must match option value exactly
    onChange={(e) => handleAssignClient(j.job_id, e.target.value)}
    style={{ padding: '4px', fontSize: '0.85rem', borderRadius: '4px' }}
  >
    <option value="">Select Client</option>
