...
const Settings = () => {
  ...
  const saveAccount = async (e) => {
    ...
    if (!currentEmail) {
      setAlert({
        color: "danger",
        text: "Missing current user email (not logged in?)",
      });
      return;
    }
  ...
  const changePassword = async () => {
    ...
    if (!email) {
      setAlert({
        color: "danger",
        text: "Email is required to change password.",
      });
      return;
    }
  ...
  const enableBrowserNotifications = async () => {
    ...
    try {
      ...
      if (permission === "granted") {
        ...
        if ("serviceWorker" in navigator) {
          try {
            ...
            setAlert({
              color: "warning",
              text: "Notifications enabled but service worker registration failed.",
            });
          }
        } else {
          setAlert({
        ...
        setAlert({
          color: "warning",
          text: "Browser notifications were blocked. You can enable them in your browser settings.",
        });
      } else {
        setAlert({
          color: "info",
          text: "Browser notification permission was not granted.",
        });
      }
    } catch (error) {
      ...
      setAlert({
        color: "danger",
        text: "Failed to enable browser notifications.",
      });
    }
  };
                    ...
                    <div className="mt-3">
                      {browserNotificationPermission !== "granted" && (
                        <>
                          ...
                          <div className="text-body-secondary settings-subtext mt-2">
                            Click to allow this site to send you desktop
                            notifications. You'll see a browser prompt asking
                            for permission.
                          </div>
                        </>
                      )}
                      ...
                      {browserNotificationPermission === "granted" && (
                        <CAlert color="success" className="mb-2">
                          ✓ Browser notifications are enabled. You'll receive
                          desktop notifications even when the app is closed.
                        </CAlert>
                      )}
                    </div>
                  )}
