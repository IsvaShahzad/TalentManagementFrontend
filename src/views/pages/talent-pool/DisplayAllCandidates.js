...
//   // const handleSignInClick = async (candidate) => {
...
//   //   try {
//   //     const data = await generateRedactedResume(candidate.candidate_id);
//   //     setRedactedUrl(data.redactedUrl);
//   //     showCAlert('redacted resume generated', 'success');
//   //   } catch (err) {
...
import CIcon from '@coreui/icons-react'
import { cilTrash, cilPencil, cilSearch, cilCloudUpload, cilBook, cilSpreadsheet } from '@coreui/icons'
import { assignClientToCandidate,
  deleteCandidateApi,
  ...
  getRecruiterCandidatesApi } from '../../../api/api'
import SavedSearch from './SavedSearch'
import Notes from './Notes'
import BulkUpload from './BulkUpload'
import { getAllSearches } from '../../../api/api';
...
const refreshCandidates = async () => {
  try {
    ...
    // Check if we are in "Recruiter Mode"
    if (passedRole === 'Recruiter' || currentUser?.role === 'Recruiter') {
      const targetId = passedRecruiterId || currentUser?.user_id;
      
      // Use your existing API function for recruiters
      // This ensures they ONLY see their own candidates
